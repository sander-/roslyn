...
function Restore-OptProfData() {
    ...
    $patAuth = if ($vsDropAccessToken) { "--patAuth `"$vsDropAccessToken`"" } else { "" }
    ...
    Move-Item -Force -Path (Join-Path $IbcOptimizationDataDir "Common7\IDE\CommonExtensions\Microsoft\ManagedLanguages\VBCSharp") -Destination $IbcOptimizationDataDir
}
